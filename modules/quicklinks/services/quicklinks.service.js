...
const QuickLinks = require("../model/quicklinks.model");
...
class QuickLinksService {
    async create(data) {
        try{
            console.log("db = ",data);
            const quicklinks = await QuickLinks.query().findOne({ title:data.title ,is_deleted: 0});
            if(!(quicklinks > 0)) {
                const quicklinkscreated = await QuickLinks.query().insert({
                    "title": data.title,
                    "href": data.href,
                ...
                console.log(quicklinkscreated);
                return quicklinkscreated;   
            }
    ...
    async getAllQuickLinksOfBanner(id) {
        try{
            let campbannerid = id;
            const data = await QuickLinks.query()
